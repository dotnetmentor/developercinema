...
oauthshim.debug = config.oauth.debug;
delete config.oauth.debug;

oauthshim.init(Object.keys(config.oauth).reduce(
  function(result, key) {
    result[config.oauth[key].id] = config.oauth[key].secret;
    return result;
  }, {}
));
...
module.exports = stack(
  ...
  securityHeaders,
  setContentType,
  route(/^\/auth\/proxy/, oauthshim.request),
  route(/callback/, auth),
  route(/^\/$/, html('index')),
...
function auth(q, r, next) {
  if (q.url.match(/denied/)) return logout(q, r, next);
  r.writeHead(200, {'Set-Cookie': 'session-id=' + '42' + '; path=/;'});
  return html('index')(q, r, next);
}
