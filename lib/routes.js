...
var db = require('./db');
var session = levelsession({db: db({path: 'session'})});
...
function loadcsrf(q, r, next) {
  var cookies = cookie(q.headers.cookie);
  var token = cookies.get('ssid');
  if (!token) {
    token = csrf()
    var expires = new Date();
    expires.setMinutes(expires.getMinutes() + 1);
    r.setHeader('Set-Cookie', 'ssid' + token + '; expires='+ expires + '; path=/;');
  }
  next();
}
