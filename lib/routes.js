...
function securityHeaders(q, r, next) {
  ...
  var csp = ''+
    "script-src 'self' https://api.twitter.com https://d2oh4tlt9mrke9.cloudfront.net https://ws.sessioncam.com;"+
    "object-src none;"+
...
function appcache(q, r, next) {
  ...
  function end(err, manifest) {
    if (err) return console.log(q.url, err);
...
function fontello(q, r, next) {
  return module.exports.fontello(q, r, next);
}
...
function metrics(q, r, next) {
  ...
  function send(err, metrics) {
    ...
    function userMetrics(err, user) {
      store.db.put('metrics!' + user.username + '!' + timestamp.now(), metrics, function(err) {
        if (err) {
          r.writeHead(500);
  ...
  function send(err, cred) {
    ...
    function created(err) {
      if (!err) {
        ...
        console.log('Failed to create user %s, error %s', cred.username, err);
    ...
    function generatedToken(err, value) {
      if (!err) {
        console.log('Should email token %s', host.baseUrl + '/verify?token=' + value);
        store.db.put('tokens!' + value, cred.username, function done() {
          users.put(cred.username, {verified: false, token: value}, invalidLogin.bind(null, r, 'email_sent'));
        });
      } else {
        console.log('Failed to create token for user %s', cred.username);
        return invalidLogin(r, 'User creation failed');
      }
    }
  }
...
function sendReset(q, r, next) {
  ...
  function send(err, cred) {
    users.get(cred.username, function (err, user) {
      ...
      function generatedToken(err, value) {
        if (!err) {
          console.log('Should email reset token %s', host.baseUrl + '/reset?token=' + value);
          store.db.put('reset!' + value, cred.username, invalidLogin.bind(null, r, 'email_sent'));
        } else {
          console.log('Failed to create token for user %s', cred.username);
